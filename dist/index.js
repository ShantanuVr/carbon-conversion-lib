...
// src/schema.ts
var FactorSourceSchema = external_exports.object({
  name: external_exports.string().min(1, "Source name is required"),
  url: external_exports.string().url().optional(),
  publishedAt: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid ISO date format").optional(),
  note: external_exports.string().optional()
});
var FactorSchema = external_exports.object({
  id: external_exports.string().min(1, "Factor ID is required"),
  region: external_exports.string().min(2, "Region code must be at least 2 characters").max(10, "Region code too long"),
  scope: external_exports.enum(["operational", "marginal", "baseline"]),
  gas: external_exports.literal("CO2e"),
  valueKgPerKWh: external_exports.number().positive("Emission factor must be positive").max(2, "Emission factor seems unreasonably high (>2 kg CO\u2082e/kWh)"),
  effectiveFrom: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid ISO date format"),
  effectiveTo: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid ISO date format").optional(),
  source: FactorSourceSchema,
  uncertaintyPct: external_exports.number().min(0).max(100).optional(),
  methodology: external_exports.string().optional(),
  version: external_exports.string().min(1, "Version is required")
});
var AvoidedInputSchema = external_exports.object({
  energy: external_exports.number().nonnegative("Energy must be non-negative"),
  inputUnit: external_exports.enum(["kWh", "MWh", "GWh"]),
  factorKgPerKWh: external_exports.number().positive("Emission factor must be positive"),
  rounding: external_exports.object({
    decimals: external_exports.number().int().min(0).max(10).optional(),
    mode: external_exports.enum(["HALF_UP", "DOWN", "TRUNC"]).optional()
  }).optional()
});
external_exports.object({
  region: external_exports.string().optional(),
  date: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid ISO date format").optional(),
  scope: external_exports.enum(["operational", "marginal", "baseline"]).optional(),
  overrides: external_exports.array(FactorSchema).optional()
});
external_exports.object({
  region: external_exports.string().optional(),
  scope: external_exports.string().optional(),
  from: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid ISO date format").optional(),
  to: external_exports.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Invalid ISO date format").optional()
});
var FactorPackSchema = external_exports.array(FactorSchema);
var RegionMappingSchema = external_exports.object({
  code: external_exports.string(),
  name: external_exports.string(),
  iso2: external_exports.string().length(2).optional(),
  iso3: external_exports.string().length(3).optional(),
  unM49: external_exports.string().optional()
});
var RegionPackSchema = external_exports.array(RegionMappingSchema);
function validateFactorPack(json) {
  return FactorPackSchema.parse(json);
}
function validateRegionPack(json) {
  return RegionPackSchema.parse(json);
}
function validateFactor(factor) {
  return FactorSchema.parse(factor);
}
function validateAvoidedInput(input) {
  return AvoidedInputSchema.parse(input);
}

// src/util/units.ts
function kWhToMWh(kWh) {
  return kWh / 1e3;
}
function MWhToKWh(MWh) {
  return MWh * 1e3;
}
function GWhToKWh(GWh) {
  return GWh * 1e6;
}
...
function normalizeToKWh(energy, unit) {
  switch (unit) {
    case "kWh":
      return energy;
    case "MWh":
...
function normalizeToKgCO2e(mass, unit) {
  switch (unit) {
    case "kgCO2e":
      return mass;
    case "tCO2e":
...
function normalizeToTonnesCO2e(mass, unit) {
  switch (unit) {
    case "kgCO2e":
...
function getEnergyConversionFactor(unit) {
  switch (unit) {
    case "kWh":
      return 1;
    case "MWh":
...
function getMassConversionFactor(unit) {
  switch (unit) {
    case "kgCO2e":
      return 1;
    case "tCO2e":
...
// src/util/math.ts
function roundStable(x, decimals, mode = "HALF_UP") {
...
function calculatePercentage(value, total, decimals = 2, mode = "HALF_UP") {
  if (total === 0) return 0;
  const percentage = value / total * 100;
  return roundStable(percentage, decimals, mode);
}
function calculateUncertaintyBounds(value, uncertaintyPct, decimals = 6, mode = "HALF_UP") {
...
// src/util/hash.ts
function stableStringify(obj, maxDecimals = 6) {
  const normalized = normalizeObject(obj, maxDecimals);
  return JSON.stringify(normalized);
}
function normalizeObject(obj, maxDecimals) {
  ...
  if (typeof obj === "object" && obj !== null) {
    ...
    for (const key of keys) {
      sortedObj[key] = normalizeObject(obj[key], maxDecimals);
    }
    return sortedObj;
  }
  return obj;
  ...
  for (const key of sortedKeys) {
    ...
    if (typeof value === "number") {
      ...
      normalized[key] = value.map(
        (item) => typeof item === "object" && item !== null ? normalizePayload(item) : item
      );
...
// src/methods/grid.ts
function toCO2eKg(energy, unit, factorKgPerKWh) {
...
function toCO2eTonnes(energy, unit, factorKgPerKWh) {
  const co2eKg = toCO2eKg(energy, unit, factorKgPerKWh);
  return co2eKg / 1e3;
}
function toCO2eTonnesRounded(energy, unit, factorKgPerKWh, decimals = 6, roundingMode = "HALF_UP") {
  const co2eTonnes = toCO2eTonnes(energy, unit, factorKgPerKWh);
  return roundStable(co2eTonnes, decimals, roundingMode);
}
function toCO2eKgRounded(energy, unit, factorKgPerKWh, decimals = 6, roundingMode = "HALF_UP") {
  const co2eKg = toCO2eKg(energy, unit, factorKgPerKWh);
  return roundStable(co2eKg, decimals, roundingMode);
}
function toCO2eWithUncertainty(energy, unit, factorKgPerKWh, uncertaintyPct, decimals = 6) {
...
function batchToCO2eTonnes(inputs) {
  return inputs.map(
    (input) => toCO2eTonnes(input.energy, input.unit, input.factorKgPerKWh)
  );
}
function totalCO2eTonnes(inputs, decimals = 6, roundingMode = "HALF_UP") {
...
// src/methods/avoided.ts
function avoidedEmissions(input) {
...
function avoidedEmissionsWithUncertainty(input, uncertaintyPct) {
...
function batchAvoidedEmissions(inputs) {
  return inputs.map((input) => avoidedEmissions(input));
}
function totalAvoidedEmissions(inputs, rounding) {
  ...
  const weightedFactor = totalEnergyKWh > 0 ? individualResults.reduce((sum, result) => sum + result.factorKgPerKWh * result.energyKWh, 0) / totalEnergyKWh : 0;
...
function avoidedEmissionsWithMethodology(input, methodology, adjustments) {
...
function resolveFactor(options) {
  ...
  let candidates = factorPack.filter(
    (factor) => factor.region === normalizedRegion && factor.scope === scope
  );
  if (candidates.length === 0 && normalizedRegion !== "WORLD") {
    candidates = factorPack.filter(
      (factor) => factor.region === "WORLD" && factor.scope === scope
    );
  }
  ...
  if (date) {
    ...
    candidates = candidates.filter((factor) => {
      ...
      const toDate = factor.effectiveTo ? new Date(factor.effectiveTo) : /* @__PURE__ */ new Date("2099-12-31");
...
function listFactors(filter) {
  ...
  if (filter.from || filter.to) {
    factors = factors.filter((factor) => {
      ...
      const factorTo = factor.effectiveTo ? new Date(factor.effectiveTo) : /* @__PURE__ */ new Date("2099-12-31");
      if (filter.from) {
...
function getAvailableScopes(region) {
  let factors = defaultFactorPack;
  if (region) {
    const normalizedRegion = normalizeRegionCode(region);
    factors = factors.filter((factor) => factor.region === normalizedRegion);
...
function getRegionInfo(regionCode) {
  return regionMappings.find(
    (region) => region.code === regionCode || region.iso2 === regionCode || region.iso3 === regionCode || region.unM49 === regionCode
  ) || null;
}
...
function getFactorsByRegionAndScope(region, scope) {
  const normalizedRegion = normalizeRegionCode(region);
  return defaultFactorPack.filter(
    (factor) => factor.region === normalizedRegion && factor.scope === scope
  );
}
function getLatestFactor(region, scope) {
...
exports.GWhToKWh = GWhToKWh;
exports.InvalidFactorError = InvalidFactorError;
exports.InvalidUnitError = InvalidUnitError;
exports.MWhToKWh = MWhToKWh;
exports.NoFactorForDateError = NoFactorForDateError;
exports.UnknownRegionError = UnknownRegionError;
exports.approximatelyEqual = approximatelyEqual;
exports.avoidedEmissions = avoidedEmissions;
exports.avoidedEmissionsWithMethodology = avoidedEmissionsWithMethodology;
exports.avoidedEmissionsWithUncertainty = avoidedEmissionsWithUncertainty;
exports.batchAvoidedEmissions = batchAvoidedEmissions;
exports.batchToCO2eTonnes = batchToCO2eTonnes;
exports.calculatePercentage = calculatePercentage;
exports.calculateUncertaintyBounds = calculateUncertaintyBounds;
exports.clamp = clamp;
exports.createDigest = createDigest;
exports.formatFixed = formatFixed;
exports.getAvailableRegions = getAvailableRegions;
exports.getAvailableScopes = getAvailableScopes;
exports.getEnergyConversionFactor = getEnergyConversionFactor;
exports.getFactorById = getFactorById;
exports.getFactorsByRegionAndScope = getFactorsByRegionAndScope;
exports.getLatestFactor = getLatestFactor;
exports.getMassConversionFactor = getMassConversionFactor;
exports.getRegionInfo = getRegionInfo;
exports.initializeDefaultFactors = loadFactorPack;
exports.initializeFactorRegistry = initializeFactorRegistry;
exports.isRegionSupported = isRegionSupported;
exports.kWhToGWh = kWhToGWh;
exports.kWhToMWh = kWhToMWh;
exports.kgToTonnes = kgToTonnes;
exports.listFactors = listFactors;
exports.loadFactorPack = loadFactorPack;
exports.normalizeToKWh = normalizeToKWh;
exports.normalizeToKgCO2e = normalizeToKgCO2e;
exports.normalizeToTonnesCO2e = normalizeToTonnesCO2e;
exports.parseFixed = parseFixed;
exports.resolveFactor = resolveFactor;
exports.roundStable = roundStable;
exports.safeAdd = safeAdd;
exports.safeMultiply = safeMultiply;
exports.safeProduct = safeProduct;
exports.safeSum = safeSum;
exports.stableEqual = stableEqual;
exports.stableStringify = stableStringify;
exports.toCO2eKg = toCO2eKg;
exports.toCO2eKgRounded = toCO2eKgRounded;
exports.toCO2eTonnes = toCO2eTonnes;
exports.toCO2eTonnesRounded = toCO2eTonnesRounded;
exports.toCO2eWithUncertainty = toCO2eWithUncertainty;
exports.toHashString = toHashString;
exports.tonnesToKg = tonnesToKg;
exports.totalAvoidedEmissions = totalAvoidedEmissions;
exports.totalCO2eTonnes = totalCO2eTonnes;
exports.validateAvoidedInput = validateAvoidedInput;
exports.validateFactor = validateFactor;
exports.validateFactorPack = validateFactorPack;
exports.validateRegionPack = validateRegionPack;
